...
public class NettyServerConfig implements Cloneable {
    ...
    /**
     * Netty public 任务线程池线程个数。（Netty网络设计，根据业务类型会创建不同的线程池，比如处理消息发送、消息消费、心跳检测等。如果该业务类型（RequestCode）未注册线程池，则由public线程处理）
     */
    private int serverCallbackExecutorThreads = 0;

    /**
     * IO线程池线程个数。（主要处理网络请求，解析请求包，包括转发到各个业务线程池完成具体的业务操作，然后将结果再返回调用方）
     */
    ...
    /**
     * 连接最大空闲时间，默认20秒。如果连接空闲时间超过该参数值，连接将被关闭。
     */
